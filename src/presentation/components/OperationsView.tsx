...
type OperationTab = 'debts' | 'returns' | 'writeoffs' | 'transfers';
...
export const OperationsView: React.FC = () => {
  ...
  const tabs = [
    ...
    { id: 'transfers' as const, label: 'Перемещения', icon: Truck, color: 'text-blue-600', bg: 'bg-blue-50' },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
        <div className="flex items-center gap-5">
           ...
           <div>
             ...
             <p className="text-[#5A5A40]/50 mt-1 text-[10px] uppercase tracking-[0.2em] italic">Управление возвратами, списаниями, перемещениями и задолженностями</p>
           </div>
        </div>

        <div className="flex bg-white/50 backdrop-blur-md p-1.5 rounded-[2rem] border border-white shadow-sm flex-wrap gap-1">
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-5 py-3 rounded-[1.5rem] transition-all duration-300 ${
                  isActive 
