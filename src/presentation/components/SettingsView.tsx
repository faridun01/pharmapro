      ...
      {isAdmin && (
        <div className="bg-white p-6 rounded-2xl border border-[#5A5A40]/10 space-y-5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-[#5A5A40]">
              <Building2 size={18} />
              <h3 className="text-lg font-bold">{t('Company Details')}</h3>
            </div>
            <button onClick={saveCompanyProfile} disabled={savingCompany} className="px-4 py-2 rounded-xl bg-[#5A5A40] text-white text-sm font-semibold disabled:opacity-50">
              {savingCompany ? t('Saving...') : t('Save company details')}
            </button>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Pharmacy name')} value={companyProfile.pharmacyName} onChange={(e) => setCompanyProfile((s) => ({ ...s, pharmacyName: e.target.value }))} />
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Legal name')} value={companyProfile.legalName} onChange={(e) => setCompanyProfile((s) => ({ ...s, legalName: e.target.value }))} />
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Address')} value={companyProfile.address} onChange={(e) => setCompanyProfile((s) => ({ ...s, address: e.target.value }))} />
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Tax ID')} value={companyProfile.taxId} onChange={(e) => setCompanyProfile((s) => ({ ...s, taxId: e.target.value }))} />
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Phone')} value={companyProfile.phone} onChange={(e) => setCompanyProfile((s) => ({ ...s, phone: e.target.value }))} />
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Email Address')} value={companyProfile.email} onChange={(e) => setCompanyProfile((s) => ({ ...s, email: e.target.value }))} />
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Director name')} value={companyProfile.directorName} onChange={(e) => setCompanyProfile((s) => ({ ...s, directorName: e.target.value }))} />
            <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Chief accountant')} value={companyProfile.chiefAccountantName} onChange={(e) => setCompanyProfile((s) => ({ ...s, chiefAccountantName: e.target.value }))} />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_.8fr] gap-5">
            <div className="space-y-4">
              <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder={t('Report prepared by')} value={companyProfile.reportPreparedBy} onChange={(e) => setCompanyProfile((s) => ({ ...s, reportPreparedBy: e.target.value }))} />
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder="Гриф утверждения" value={companyProfile.approvalTitle} onChange={(e) => setCompanyProfile((s) => ({ ...s, approvalTitle: e.target.value }))} />
                <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder="Должность утверждающего" value={companyProfile.approvalRole} onChange={(e) => setCompanyProfile((s) => ({ ...s, approvalRole: e.target.value }))} />
                <input className="w-full px-4 py-2.5 border border-[#5A5A40]/10 rounded-xl" placeholder="Подпись / печать" value={companyProfile.stampLabel} onChange={(e) => setCompanyProfile((s) => ({ ...s, stampLabel: e.target.value }))} />
              </div>
            </div>

            <div className="rounded-2xl border border-dashed border-[#5A5A40]/20 p-4 bg-[#fafaf7] space-y-4">
              <div>
                <p className="text-sm font-bold text-[#5A5A40]">Логотип для печатного шаблона</p>
              ...
              <div className="rounded-2xl bg-white border border-[#5A5A40]/10 min-h-40 flex items-center justify-center overflow-hidden">
                {companyProfile.logoDataUrl ? (
                  ...
                  <div className="text-center text-[#5A5A40]/45 px-4">
                    <ImageUp size={24} className="mx-auto mb-2" />
                    <p className="text-sm">Логотип не загружен</p>
                  </div>
                )}
              </div>

              <div className="flex flex-wrap gap-3">
                <label className="px-4 py-2 rounded-xl bg-[#5A5A40] text-white text-sm font-semibold cursor-pointer inline-flex items-center gap-2">
                  <ImageUp size={14} /> Загрузить логотип
                  <input type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" className="hidden" onChange={(event) => { void handleLogoUpload(event); }} />
                </label>
                <button
                  type="button"
                  onClick={() => setCompanyProfile((state) => ({ ...state, logoDataUrl: '' }))}
                  disabled={!companyProfile.logoDataUrl}
                  className="px-4 py-2 rounded-xl border border-[#5A5A40]/20 text-[#5A5A40] text-sm font-semibold disabled:opacity-50 inline-flex items-center gap-2"
                >
      ...
      {isAdmin && (
        <div className="bg-white p-6 rounded-2xl border border-[#5A5A40]/10 space-y-5">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="text-lg font-bold text-[#5A5A40]">{t('Stock Integrity')}</h4>
              <p className="text-sm text-[#5A5A40]/60">{t('Validate and repair stock consistency across product, batch, and warehouse tables')}</p>
            </div>
            <ShieldAlert size={22} className="text-[#5A5A40]/60" />
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => { void checkIntegrity(false); }}
              disabled={integrityLoading}
              className="px-4 py-2.5 rounded-xl bg-[#5A5A40] text-white text-sm font-semibold disabled:opacity-50 flex items-center gap-2"
            >
              <RefreshCw size={16} />
              {integrityLoading ? t('Checking...') : t('Check Integrity')}
            </button>
            <button
              onClick={fixIntegrity}
              disabled={integrityLoading}
              className="px-4 py-2.5 rounded-xl border border-[#5A5A40]/20 text-[#5A5A40] text-sm font-semibold disabled:opacity-50 flex items-center gap-2 hover:bg-[#f5f5f0]"
            >
              <Wrench size={16} />
              {t('Repair Integrity')}
            </button>
          ...
          {integrityReport && (
            <div className="space-y-3">
              {autoChecked && (
                <div className="rounded-xl p-2.5 text-xs border bg-[#f5f5f0] border-[#5A5A40]/10 text-[#5A5A40]/70">
                  {t('Auto-check on page load completed')}
                </div>
              )}
              <div className={`rounded-xl p-3 text-sm border ${integrityReport.ok ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-amber-50 border-amber-200 text-amber-700'}`}>
                <div className="flex items-center gap-2 font-semibold">
                  <CheckCircle2 size={16} />
                  {integrityReport.ok ? t('No stock integrity issues found') : `${t('Found issues')}: ${integrityReport.issuesCount}`}
                </div>
                <div className="mt-1 text-xs">
                  {t('Checked products')}: {integrityReport.checkedProducts} | {t('Warehouse rows')}: {integrityReport.checkedWarehouseStockRows}
                </div>
              </div>
            </div>
          )}
      ...
      {isAdmin && (
        <div className="bg-white p-6 rounded-2xl border border-[#5A5A40]/10 flex items-center justify-between">
          <div className="flex items-center gap-3 text-[#5A5A40]">
            <Database size={18} />
            <div>
              <p className="font-bold">{t('Backup & Restore')}</p>
              <p className="text-sm text-[#5A5A40]/60">{t('Export full operational backup as JSON')}</p>
            </div>
          </div>
          <button onClick={exportBackup} disabled={backupLoading} className="px-4 py-2 rounded-xl border border-[#5A5A40]/20 text-[#5A5A40] text-sm font-semibold disabled:opacity-50">
            {backupLoading ? t('Exporting...') : t('Export Backup')}
          </button>
        </div>
      )}
    </div>
