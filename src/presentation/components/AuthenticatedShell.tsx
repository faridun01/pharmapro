...
type View = 'dashboard' | 'notifications' | 'pos' | 'inventory' | 'batches' | 'invoices' | 'debtors' | 'purchase-invoices' | 'suppliers' | 'reports' | 'settings' | 'returns' | 'writeoffs' | 'shifts';
...
const PurchaseInvoicesView = lazyNamedImport(() => import('./PurchaseInvoicesView'), 'PurchaseInvoicesView');
...
export default function AuthenticatedShell({ onSignedOut }: { onSignedOut?: () => void }) {
  ...
  const menuItems = [
    ...
    { id: 'purchase-invoices', label: 'Приходные', icon: ClipboardList },
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'purchase-invoices': return <PurchaseInvoicesView />;
